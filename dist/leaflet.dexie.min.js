...
this._resetStatus(tiles);this.status.currMinZoom=zoomlevels[0];var self=this;var saveCallback=function(tblName){self._baseLayer.fire('savestart',self.status);var subdlength=self._baseLayer.getSimultaneous();for(var i=0;i<subdlength;i++){self._loadTile(tblName)}};if(this.options.confirmSave){this.options.confirmSave(this.status,saveCallback)}else{saveCallback()}},setStorageSize:function(callback){var self=this;this.dtable.count().then(function(numberOfKeys){self.status.storagesize=numberOfKeys;self._baseLayer.fire('storagesize',self.status);if(callback){callback(numberOfKeys)}},function(err){callback(0);throw err})},_resetStatus:function(tiles){this.status.lengthLoaded=0;this.status.lengthToBeSaved=tiles.length;this.status.lengthSaved=0;this.status._tilesforSave=tiles},_loadTile:async function(tblName){var self=this;var tileUrl=self.status._tilesforSave.shift();const blob=await fetch(tileUrl.url).then((res)=>{return(res.blob())}).catch(err=>console.log(err));if(!blob)return;if(self.status.lengthLoaded==0){if(self.status.tnames.indexOf(tblName)<0)
...
self._saveTile(tileUrl.key,blob);self.status.mapSize+=blob.size;self.status.lengthLoaded++;if(self.status._tilesforSave.length>0){self._loadTile(tblName).catch(err=>console.log(err))}else{if(self.status.lengthLoaded===self.status.lengthToBeSaved){console.log("New table "+self.dtable.name);self._baseLayer.fire('loadend',self.status)}}},_saveTile:function(tileUrl,blob){var self=this;if(!this.dtable)return;this.dtable.put(blob,tileUrl).then(()=>{self.status.lengthSaved++;self._baseLayer.fire('savetileend',self.status);if(self.status.lengthSaved===self.status.lengthToBeSaved){self._baseLayer.fire('tblevent',self.status);self.setStorageSize()}}).catch(err=>console.log(err))},_extendSchema:async function(tbl){this._db.close();var currSchema=this.status.tnames.reduce(function(obj,v){obj[v]='';return obj},{})
